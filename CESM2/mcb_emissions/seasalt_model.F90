...
contains
  ...
  subroutine seasalt_init(seasalt_emis_scale)
    ...
    use mcb_ssa_flux, only: read_flux_mcb
  ...
  !=============================================================================
  !=============================================================================
  ! add state as an argument for MCB
  subroutine seasalt_emis(state, u10cubed,  srf_temp, ocnfrc, ncol, cflx )
    use physics_types, only: physics_state ! for MCB
    use sslt_sections, only: nsections, fluxes, Dg, rdry
    use mo_constants,  only: dns_aer_sst=>seasalt_density, pi
    use mcb_ssa_flux,   only: get_fi_mcb ! for MCB
    
    ! dummy arguments
    type(physics_state), intent(in) :: state ! for MCB
    ...
    ! From Mingxuan - Get sea salt flux
    fi_mcb(:,:) = 0.0_r8
    call get_fi_mcb( lchnk, ncol, fi_mcb )
    ...
    do ibin = 1,nslt
       ...
       if (mn>0) then
          do i=1, nsections
             if (Dg(i).ge.sst_sz_range_lo(ibin) .and. Dg(i).lt.sst_sz_range_hi(ibin)) then
                ...
                cflx(:ncol,mn)=cflx(:ncol,mn)+(fi(:ncol,i) + fi_mcb(:ncol,i))*ocnfrc(:ncol)*emis_scale
             endif
          enddo
       endif
       ...
       do i=1, nsections
          if (Dg(i).ge.sst_sz_range_lo(ibin) .and. Dg(i).lt.sst_sz_range_hi(ibin)) then
             ...
             cflx(:ncol,mm)=cflx(:ncol,mm)+(fi(:ncol,i) + fi_mcb(:ncol,i))*ocnfrc(:ncol)*emis_scale  &   !++ ag: scale sea-salt
                  *4._r8/3._r8*pi*rdry(i)**3*dns_aer_sst  ! should use dry size, convert from number to mass flux (kg/m2/s)
          endif
       enddo
    enddo
