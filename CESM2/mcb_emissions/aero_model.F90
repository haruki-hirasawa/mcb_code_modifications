...
contains
  ...
  subroutine aero_model_readnl(nlfile)
    use mcb_ssa_flux,    only: mcb_readnl
...
!
  ...
  subroutine aero_model_emissions( state, cam_in )
    ...
    if (seasalt_active) then
       ...
       ! from Mingxuan - add state as an argument
       call seasalt_emis(state, u10cubed, cam_in%sst, cam_in%ocnfrac, ncol, cam_in%cflx )
